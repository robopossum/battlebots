...
# rule 1
    ...
    def makeRandomMove(self, action):
        ...
        choice = (random.choice(['up', 'up', 'up', 'turnLeft', 'turnRight']))
#        print("Choice is ", choice)
    ...
    def navigateToExp(self, fact):
        ...
        def getMove2(self):
            stateDf = pd.DataFrame.from_records(self.state['sensors'])
            target = stateDf[stateDf['length'] == min(stateDf['length'])]
            target = target[target['hitting'] == 4]
            self.stateDf = stateDf
            if target.angle > -6 and target.angle < 6:
                print("go forwards")
                return 'up'
            elif int(target.angle) < -6:
                print("Turn Left!")
                return 'left'
            elif int(target.angle) > 6:
                print("Turn right!")
